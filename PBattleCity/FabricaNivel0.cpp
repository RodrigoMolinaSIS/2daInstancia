...
void FabricaNivel0::crearInstanciasMapaNivel()
{
	...
	for (int f = 0; f < getMapaNivel().size(); f++) {
		for (int c = 0; c < getMapaNivel()[f].size(); c++) {
			...
			case celdaSimbolo_GloboAerostatico: {
				GloboAerostatico* globoAerostatico = (GloboAerostatico*)crearInstanciaGloboAerostatico(c, f);
				globoAerostatico-> setFisico(false);
				break;
			}
...
//globo
Actor* FabricaNivel0::crearInstanciaGloboAerostatico(float _x, float _y) {
	Actor* actor = new GloboAerostatico();
	actor->setAvatar(avatarGloboAerostatico);
	actor->setX(_x);
	actor->setY(_y);
	actor->setGameManager(getGameManager());
	/*if (getGameManager()->moverActorA(actor, _x, _y) == false)
